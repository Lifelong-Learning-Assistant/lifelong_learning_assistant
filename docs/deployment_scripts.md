...
В проекте используются скрипты `start-dev.sh` и `stop-dev.sh` для управления жизненным циклом контейнеров в режиме разработки (DEV). Эти скрипты обеспечивают правильный порядок запуска, группировку сервисов и управление зависимостями.
